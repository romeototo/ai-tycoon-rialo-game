...
const achievements = [
  {
    id: "first_tap",
    name: "🖱️ First Tap",
    desc: "Tap the node for the first time",
    check: () => stats.totalTaps >= 1,
    done: false,
  },
  {
    id: "rich_100",
    name: "💰 First Hundred",
    desc: "Earn 100 $RIA",
    check: () => stats.totalEarned >= 100,
    done: false,
  },
  {
    id: "rich_10k",
    name: "💎 10K Club",
    desc: "Earn 10,000 $RIA",
    check: () => stats.totalEarned >= 10000,
    done: false,
  },
  {
    id: "rich_1m",
    name: "🏆 Millionaire",
    desc: "Earn 1,000,000 $RIA",
    check: () => stats.totalEarned >= 1000000,
    done: false,
  },
  {
    id: "combo_3",
    name: "🔥 Combo Master",
    desc: "Reach combo x3.0",
    check: () => stats.bestCombo >= 3,
    done: false,
  },
  {
    id: "combo_5",
    name: "💥 Combo Legend",
    desc: "Reach combo x5.0",
    check: () => stats.bestCombo >= 5,
    done: false,
  },
  {
    id: "boss_1",
  ...
  {
    id: "epoch_20",
    name: "🛡️ Epoch Warrior",
    desc: "Clear 20 epochs",
    check: () => stats.epochsCleared >= 20,
    done: false,
  },
  {
    id: "epoch_50",
    name: "🌟 Epoch Legend",
    desc: "Clear 50 epochs",
    check: () => stats.epochsCleared >= 50,
    done: false,
  },
  {
    id: "crit_50",
...
function updateMission() {
  if (!missionTitle || !missionText || !missionReward || !goalFill || !goalText)
    return;
...
function getCostForOne(upgrade, currentCount) {
  return Math.floor(
    upgrade.baseCost * Math.pow(upgrade.costMult, currentCount),
  );
}
...
function updateUI() {
  ...
  if (nodeStatus) {
    nodeStatus.innerText =
      tokensPerSecond > 0
        ? `${formatNumber(tokensPerSecond)} $RIA/sec online`
        : "Tap to start";
  }
  ...
  if (epochFill && epochLabel) {
    ...
    epochLabel.innerText = isBossStage
      ? `⚔️ SLASH EVENT`
      : `Epoch ${currentStage}`;
  }
  ...
  upgrades.forEach((upgrade, index) => {
    ...
    if (btns[index]) {
      ...
      prices[index].className =
        `upgrade-price ${info.canAfford ? "cost-green" : "cost-red"}`;
  ...
  if (stakedAmountDisplay)
    stakedAmountDisplay.innerText = `Staked: ${formatNumber(stakedTokens)} $RIA | Multiplier: x${stakingMultiplier.toFixed(2)}`;
...
function initStage() {
  ...
  if (bossLogo) {
    bossLogo.style.filter = isBossStage
      ? "invert(30%) sepia(100%) saturate(1000%) hue-rotate(330deg)"
      : "brightness(0) invert(1)";
  }
  // Boss mode visual
  if (isBossStage) {
    aiCore.classList.add("boss-mode");
    body_screenShake();
  } else {
    aiCore.classList.remove("boss-mode");
  }
  ...
  if (stageNameElem) {
    if (isBossStage) {
      ...
      updateEventBanner(
        "Boss round: clear pending transactions before the slash timer ends",
      );
    } else {
...
aiCore.addEventListener("mousedown", (e) => {
  ...
  const actualPower =
    clickPower * combo * (isCrit ? 5 : 1) * overclockMult * feverMult;

  dealDamage(actualPower, e.clientX, e.clientY, isCrit);
  createParticles(e.clientX, e.clientY, isCrit);
  aiCore.classList.add("tapped");
  setTimeout(() => aiCore.classList.remove("tapped"), 100);

  if (feverMode) {
    comboDisplay.style.opacity = 1;
    comboDisplay.innerText = `🔥 FEVER x5! 🔥`;
...
function createFloatingNumber(x, y, amount, isCrit = false) {
  ...
  if (isCrit) {
    ...
    el.innerText = "⚡ CRIT +" + formatNumber(amount) + " $RIA";
  } else if (overclockActive) {
    el.classList.add("overclock");
    el.innerText = "🔥 +" + formatNumber(amount) + " $RIA";
  } else {
    el.classList.add("normal");
    el.innerText = "+" + formatNumber(amount) + " $RIA";
  }
  const offsetX = (Math.random() - 0.5) * 60;
  el.style.left = x + offsetX - 40 + "px";
  el.style.top = y - 30 - Math.random() * 20 + "px";
...
function buyUpgrade(upgrade, info) {
  if (tokens >= info.cost) {
    playBuy();
    tokens -= info.cost;
    upgrade.count += info.amount;
    recalculateStats();
    renderUpgrades();
    updateUI();
    saveGame();
    showToast(
      `${upgrade.name.replace(/^[^\w]+/, "")} online: +${formatNumber(upgrade.tps * info.amount)} $RIA/sec`,
    );
  }
}
...
function saveGame() {
  const saveState = {
    ...
    stats,
    achievementsDone: achievements.filter((a) => a.done).map((a) => a.id),
  };
...
function loadGame() {
  ...
  if (saved) {
    try {
      ...
      stakedTokens =
        typeof data.stakedTokens === "number" ? data.stakedTokens : 0;
      prestigeTokens =
        typeof data.prestigeTokens === "number" ? data.prestigeTokens : 0;
      goalReached = Boolean(data.goalReached);
      currentStage =
        typeof data.currentStage === "number" ? data.currentStage : 1;
      savedMonsterHP =
        typeof data.monsterHP === "number" ? data.monsterHP : null;
      if (data.upgrades)
...
if (overclockBtn) {
  overclockBtn.addEventListener("click", () => {
    ...
    overclockCooldown = OVERCLOCK_DURATION;
...
function updateLeaderboard() {
  ...
  let all = [
    ...bots,
    { name: "YOU (Validator)", score: tokens + stakedTokens, isPlayer: true },
  ];
  all.sort((a, b) => b.score - a.score);
  let html = "";
  all.slice(0, 5).forEach((p, i) => {
    if (p.isPlayer)
      html += `<strong style="color: var(--accent);">${i + 1}. ${p.name} - ${formatNumber(p.score)}</strong><br>`;
...
if (prestigeBtn) {
  prestigeBtn.addEventListener("click", () => {
    ...
    if (reward > 0) {
      ...
      if (forkDesc)
        forkDesc.innerText =
          "You will lose ALL $RIA, Staked $RIA, and Upgrades.";
      if (forkGain)
        forkGain.innerText = `+${reward} Hashpower (+${reward * 50}% permanent boost)`;
...
if (keepPlayingBtn)
  keepPlayingBtn.onclick = () => winModal.classList.remove("active");
...
// DDOS Mini Game
setInterval(() => {
  if (!isBossStage && !isDdosActive && Math.random() < 0.15 && currentStage > 2)
    startDdos();
}, 30000);
...
function startDdos() {
  ...
  alertDiv.className = "ddos-alert";
  document.body.appendChild(alertDiv);
  const ddosCounter = document.createElement("div");
  ddosCounter.className = "ddos-counter";
  ddosCounter.innerText = "Threats: 5/5";
  document.body.appendChild(ddosCounter);
  let bugsLeft = 5;
  for (let i = 0; i < 5; i++) {
    const ddosBug = document.createElement("div");
    ddosBug.className = "ddos-bug";
    ddosBug.innerText = "👾";
    ddosBug.style.left = Math.random() * (window.innerWidth - 100) + "px";
    ddosBug.style.top = Math.random() * (window.innerHeight - 100) + "px";
    ...
    ddosBug.onmousedown = (e) => {
      ...
      if (bugsLeft <= 0) {
        ...
        recalculateStats();
        updateUI();
        const reward = tokensPerSecond * 10;
        tokens += reward;
        updateUI();
        saveGame();
    ...
    ddosBug.addEventListener(
      "touchstart",
      (e) => {
        e.preventDefault();
        ddosBug.onmousedown(e.touches[0]);
      },
      { passive: false },
    );
  }
...
function checkAchievements() {
  ...
  if (newUnlocks.length > 0) {
    ...
    if (banner) {
      ...
      checkAchievements._timer = setTimeout(() => {
...
if (closeStatsBtn)
  closeStatsBtn.onclick = () => statsModal.classList.remove("active");
...
function renderStats() {
  ...
  grid.innerHTML = [
    ...
    .map(
      ([label, val]) =>
        `<div class="stat-item"><div class="stat-label">${label}</div><div class="stat-value">${val}</div></div>`,
    )
    .join("");
}
...
function saveGameWithIndicator() {
  ...
  if (saveIndicator) {
    ...
    saveGameWithIndicator._t = setTimeout(
...
document.addEventListener("keydown", (e) => {
  if (e.code === "Space" && !e.repeat) {
    ...
    aiCore.dispatchEvent(
      new MouseEvent("mousedown", { clientX: cx, clientY: cy }),
    );
  }
  ...
  try {
    ...
    if (typeof data.lastSaveTime === "number" && tokensPerSecond > 0) {
      const offlineSeconds = Math.min(
        Math.max((Date.now() - data.lastSaveTime) / 1000, 0),
        12 * 60 * 60,
      );
      if (offlineSeconds >= 30) {
        ...
        const timeStr =
          offlineSeconds >= 3600
            ? (offlineSeconds / 3600).toFixed(1) + "h"
            : offlineSeconds >= 60
              ? Math.floor(offlineSeconds / 60) + "m"
...
(function randomNodeMovement() {
  ...
  function moveNode() {
    ...
    nodeWrapper.style.transitionDuration =
      ((interval / 1000) * 0.85).toFixed(2) + "s";
  ...
  const rewards = [
    {
      label: "💰 Double Burst!",
      fn: () => {
        const r = tokensPerSecond * 5;
        tokens += r;
        stats.totalEarned += r;
        return `+${formatNumber(r)} $RIA`;
      },
    },
    {
      label: "⚡ Instant Overclock!",
      fn: () => {
        overclockActive = true;
        overclockCooldown = OVERCLOCK_DURATION;
        aiCore.classList.add("overclock-on");
        updateSkillBtn();
        return "Overclock activated!";
      },
    },
    {
      label: "🔥 Fever Trigger!",
    ...
    },
    {
      label: "💎 Jackpot!",
      fn: () => {
        const r = tokensPerSecond * 20;
        tokens += r;
        stats.totalEarned += r;
        return `+${formatNumber(r)} $RIA JACKPOT!`;
      },
    },
    {
      label: "🛡️ Epoch Skip!",
      fn: () => {
        ...
        currentStage++;
        initStage();
        return `Epoch cleared + ${formatNumber(bonus)} $RIA`;
      },
    },
  ];
...
(function coinRainSystem() {
  function startRain() {
    ...
    setTimeout(() => {
      if (collected > 0)
        showToast(
          `🌧️ Collected ${collected} coins: +${formatNumber(collected * coinValue)} $RIA`,
        );
    }, 5500);
